...
//             alt="" draggable={false} onLoad={() => setLoaded(true)}
//             style={{ opacity: loaded ? 1 : 0 }} />
//         )}
//       </div>
...
//         <button className="pg-upload-cancel" onClick={onCancel}>
//           Cancel Upload
//         </button>
//       </div>
//     </div>
...
//         setMedia(shuffle(data.media || []));
//       } catch {
//         const saved = localStorage.getItem("cloudinary_media");
//         if (saved) setMedia(shuffle(JSON.parse(saved)));
...
//           };
//           xhr.open("POST", `https://api.cloudinary.com/v1_1/${CLOUD_NAME}/${file.type.startsWith("video") ? "video" : "image"}/upload`);
//           xhr.send(formData);
//         });
...
//       });
//     } catch { }
//     setTimeout(() => { setMedia(prev => prev.filter(m => m.publicId !== item.publicId)); setDeletingId(null); }, 300);
//   }, []);
...
//         {/* ── Separate Filter Pill (Shows below on click) ── */}
//         <div className={`pg-filter-pill ${isMenuOpen ? "show" : "hide"}`}>
//           {/* Sliding highlight - accounts for the 6px gap */}
...
//       {uploading && <UploadModal progress={uploading.progress} filename={uploading.filename} onCancel={handleCancelUpload} />}
